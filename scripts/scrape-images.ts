...
const collectImage = async (species: string) => {
  const pokemonDbName = species
    ...
    .replace(/^enamorous$/, 'enamorous-incarnate')
    .replace(/^oinkologne$/, 'oinkologne-male')
    .replace(/^palafin$/, 'palafin-zero')
    .replace(/^maushold-four$/, 'maushold-family4')
    .replace(/^tauros-paldea-combat/, 'tauros-paldea')
    .replace(/^tauros-paldea-blaze/, 'tauros-blaze')
    .replace(/^tauros-paldea-aqua/, 'tauros-aqua')
